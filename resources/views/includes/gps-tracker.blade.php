{{--
    Captura automática de ubicación GPS + seguimiento "suave" del permiso.
    Se activa solo si config('security.session.track_location') = true.

    - Pide permiso al navegador. Si el usuario lo PERMITE, envía su posición
      periódicamente.
    - Si lo DENIEGA (o no está disponible), NO bloquea la navegación: muestra
      un aviso persistente y registra el estado en la bitácora (quién lo tiene
      desactivado), para seguimiento.
--}}
<script>
    (function () {
        const GPS_ENDPOINT = "{{ route('location.store-gps') }}";
        const PERM_ENDPOINT = "{{ route('location.report-permission') }}";
        const TOKEN = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');
        if (!TOKEN) return;

        const INTERVAL = {{ max(60, (int) config('security.session.track_interval', 300)) }} * 1000;
