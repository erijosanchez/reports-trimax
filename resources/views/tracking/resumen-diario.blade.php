                        ...
                        <div class="shadow-sm border-0 h-100 card">
                            <div class="card-body">
                                <div class="d-flex align-items-center justify-content-between">
                                    <div>
                                        <p class="mb-1 text-muted small">{{ $k['label'] }}</p>
                                        <h3 class="mb-0 fw-bold text-{{ $k['color'] }}">{{ $k['val'] }}</h3>
                                    </div>
                                    <div class="rounded-circle d-flex align-items-center justify-content-center bg-{{ $k['color'] }}"
                                        style="width:48px;height:48px;opacity:.85">
                                        <i class="mdi {{ $k['icon'] }} text-white" style="font-size:22px"></i>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="mb-4 col-lg-8">
                    <div class="shadow-sm border-0 card">
                        <div class="d-flex align-items-center justify-content-between card-header">
                            ...
                            <span class="bg-primary badge">{{ \Carbon\Carbon::parse($fecha)->format('d/m/Y') }}</span>
                        </div>
                                                ...
                                                <td style="min-width:140px">
                                                    ...
                                                    <div class="progress">
                                                        <div class="bg-success progress-bar"
                                                            style="width:{{ $tot > 0 ? round(($comp / $tot) * 100) : 0 }}%"></div>
                                                        <div class="bg-danger progress-bar"
                                                            style="width:{{ $tot > 0 ? round(($fall / $tot) * 100) : 0 }}%"></div>
                                                    </div>
                            ...
                            @forelse($porSede as $sede => $data)
                                <div class="mb-3">
                                    <div class="d-flex justify-content-between mb-1">
                                        ...
                                        <span class="text-primary fw-bold small">{{ $data['km'] }} km</span>
                                    </div>
                                    <div class="text-muted" style="font-size:11px">
                                        {{ $data['rutas'] }} ruta{{ $data['rutas'] !== 1 ? 's' : '' }} ·
                                        {{ $data['entregas'] }} entrega{{ $data['entregas'] !== 1 ? 's' : '' }}
                                    </div>
                                </div>
                            @empty
