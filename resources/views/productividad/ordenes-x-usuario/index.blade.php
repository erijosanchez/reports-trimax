                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-header bg-white border-bottom d-flex align-items-center justify-content-between py-3">
                        ...
                        <div class="d-flex gap-2 align-items-center">
                            <span class="badge fs-6 px-3 py-2" id="badge-pct-antes"
                                  style="background:#2563eb; font-size:.8rem !important;">
                                <i class="mdi mdi-clock-outline me-1"></i>
                                ANTES DE LAS 5 PM: <span id="badge-pct-antes-val">—</span>
                            </span>
                            <span class="badge fs-6 px-3 py-2" id="badge-pct-despues"
                                  style="background:#ef4444; font-size:.8rem !important;">
                                <i class="mdi mdi-clock-outline me-1"></i>
                                DESPUÉS DE LAS 5 PM: <span id="badge-pct-despues-val">—</span>
                            </span>
                        </div>
                    </div>
                    <div class="card-body p-2">
...
(function () {
    ...
    function renderChartHoraTotal(data) {
        ...
        // Badges antes / después 5pm
        const totalGen   = data.data.reduce((a, b) => a + b, 0);
        const antesTotal = data.labels.reduce((acc, lbl, i) => {
            return parseInt(lbl.split(':')[0]) < 17 ? acc + (data.data[i] || 0) : acc;
        }, 0);
        const despuesTotal = totalGen - antesTotal;
        const pctAntes   = totalGen > 0 ? ((antesTotal   / totalGen) * 100).toFixed(1) + '%' : '—';
        const pctDespues = totalGen > 0 ? ((despuesTotal / totalGen) * 100).toFixed(1) + '%' : '—';
        document.getElementById('badge-pct-antes-val').textContent   = pctAntes;
        document.getElementById('badge-pct-despues-val').textContent = pctDespues;
