...
{{-- ══ CABECERA ══ proporciones del Word: 25.4% | 48.3% | 9.2% | 17.1% --}}
<table class="b mt">
...
<table class="mt">
  <tr>
    ...
    <td style="width:40%; border-bottom:1pt solid #000; padding:3pt 1pt; text-align:right;">
      <b>Fecha de Solicitud:</b> {{ $requerimiento->fecha_solicitud->format('d/m/Y') }}
    </td>
  </tr>
</table>
...
<table class="b mt">
  ...
  <tr>
    ...
    <td class="c" style="width:32%;">{{ $requerimiento->puesto }}</td>
    <td class="c lbl" style="width:8%;">Área:</td>
    <td class="c" style="width:42%;">{{ $requerimiento->gerencia }}</td>
  </tr>
  <tr>
    <td class="c lbl">Reporta a :</td>
    <td class="c" colspan="3">{{ $requerimiento->jefe_directo }}</td>
  </tr>
  <tr>
    <td class="c lbl">Supervisa a :</td>
    <td class="c" colspan="3">{{ $requerimiento->supervisa_a ?? '' }}</td>
  </tr>
  <tr>
    <td class="c lbl">N° de vacantes :</td>
    <td class="c">{{ $requerimiento->num_vacantes ?? 1 }}</td>
...
{{-- ══ SECCIONES 2 + 3 ══ proporciones: 47.9% | 52.1% --}}
<table class="b mt">
  <tr>
    {{-- SECCIÓN 2 --}}
      ...
      <table>
        ...
        <tr>
          <td class="c" style="width:50%;">
            <span class="chk">{{ $requerimiento->tipo_vacante==='vacante' ? 'X':'&nbsp;' }}</span> Vacante
          </td>
          <td class="c" style="width:50%;">
            <span class="chk">{{ $requerimiento->permanencia==='temporal' ? 'X':'&nbsp;' }}</span> Temporal
          </td>
        </tr>
        <tr>
          <td class="c">
            <span class="chk">{{ $requerimiento->tipo_vacante==='reemplazo' ? 'X':'&nbsp;' }}</span> Reemplazo
          </td>
          <td class="c">
            <span class="chk">{{ $requerimiento->permanencia==='permanente' ? 'X':'&nbsp;' }}</span> Permanente
          </td>
        </tr>
        <tr>
          <td class="c">
            <span class="chk">{{ $requerimiento->tipo_vacante==='posicion_nueva' ? 'X':'&nbsp;' }}</span> Posición nueva
          </td>
          <td class="c">
            <span class="chk">{{ $requerimiento->disponibilidad_viaje ? 'X':'&nbsp;' }}</span> Disp. para viajar
          </td>
        </tr>
        <tr>
          <td class="c">
            <span class="chk">{{ $requerimiento->jornada==='tiempo_parcial' ? 'X':'&nbsp;' }}</span> Tiempo Parcial
          </td>
          <td class="c">
            <span class="chk">{{ $requerimiento->jornada==='tiempo_completo' ? 'X':'&nbsp;' }}</span> Tiempo Completo
          </td>
        </tr>
        <tr>
          <td class="c" colspan="2"><b>Motivo</b> &nbsp; {{ $requerimiento->motivo ?? '' }}</td>
        </tr>
      </table>
    </td>
    ...
    <td style="width:52.1%; vertical-align:top; padding:0;">
      <table>
        ...
        <tr>
          ...
          <td class="c">{{ $requerimiento->fecha_estimada_contratacion ? $requerimiento->fecha_estimada_contratacion->format('d/m/Y') : '' }}</td>
        </tr>
        <tr>
          <td class="c lbl">Tipo de contrato</td>
          <td class="c">{{ $requerimiento->tipo_contrato ?? '' }}</td>
        </tr>
        <tr>
          <td class="c lbl">Duración</td>
          <td class="c">{{ $requerimiento->duracion_contrato ?? '' }}</td>
        </tr>
        <tr>
          <td class="c lbl">Remuneración prevista &nbsp; S/.</td>
          <td class="c">{{ $requerimiento->remuneracion_prevista ? number_format($requerimiento->remuneracion_prevista, 2) : '' }}</td>
        </tr>
        <tr>
          <td class="c lbl">Horario de Trabajo</td>
          <td class="c">{{ $requerimiento->horario_trabajo ?? '' }}</td>
        </tr>
        <tr>
          <td class="c lbl">Beneficios</td>
          <td class="c">{{ $requerimiento->beneficios ?? '' }}</td>
        </tr>
      </table>
    </td>
  </tr>
...
{{-- ══ SECCIÓN 4: CANDIDATOS ══ proporciones: 72.7% | 27.3% --}}
<table class="b mt">
  ...
  <tr>
    <td class="c">{{ $l }}. {{ $cands[$i]['nombre'] ?? '' }}</td>
    <td class="c">{{ $cands[$i]['telefono'] ?? '' }}</td>
  </tr>
  @endforeach
</table>
...
<table class="b mt">
  ...
  @foreach(['a','b','c'] as $i => $l)
  <tr><td class="c">{{ $l }}. {{ $herrs[$i] ?? '' }}</td></tr>
  @endforeach
</table>

{{-- ══ SECCIÓN 6: FIRMAS ══ proporciones: 32.3% | 30.0% | 37.7% --}}
<table class="b mt">
    ...
    <td style="width:32.3%; text-align:center; border:1pt solid #000; padding:4pt 3pt;">
      <div class="firma-esp">
        @if($requerimiento->firma_solicitante_data)
          <img src="{{ $requerimiento->firma_solicitante_data }}" alt="firma" style="max-height:50pt; max-width:100pt;">
        @endif
      </div>
      <div class="firma-line"></div>
      <div class="firma-lbl">Responsable de Área Solicitante</div>
      @if($requerimiento->firma_solicitante_nombre)<div class="firma-nom">{{ $requerimiento->firma_solicitante_nombre }}</div>@endif
      @if($requerimiento->firma_solicitante_at)<div class="firma-fec">{{ $requerimiento->firma_solicitante_at->format('d/m/Y H:i') }}</div>@endif
    </td>
    <td style="width:30.0%; text-align:center; border:1pt solid #000; padding:4pt 3pt;">
      <div class="firma-esp">
        @if($requerimiento->firma_rrhh_data)
          <img src="{{ $requerimiento->firma_rrhh_data }}" alt="firma" style="max-height:50pt; max-width:100pt;">
        @endif
      </div>
      <div class="firma-line"></div>
      <div class="firma-lbl">Responsable de Recursos Humanos</div>
      @if($requerimiento->firma_rrhh_nombre)<div class="firma-nom">{{ $requerimiento->firma_rrhh_nombre }}</div>@endif
      @if($requerimiento->firma_rrhh_at)<div class="firma-fec">{{ $requerimiento->firma_rrhh_at->format('d/m/Y H:i') }}</div>@endif
    </td>
    <td style="width:37.7%; text-align:center; border:1pt solid #000; padding:4pt 3pt;">
      <div class="firma-esp">
        @if($requerimiento->firma_gerente_data)
          <img src="{{ $requerimiento->firma_gerente_data }}" alt="firma" style="max-height:50pt; max-width:100pt;">
        @endif
      </div>
      <div class="firma-line"></div>
      <div class="firma-lbl">Gerente General</div>
      @if($requerimiento->firma_gerente_nombre)<div class="firma-nom">{{ $requerimiento->firma_gerente_nombre }}</div>@endif
      @if($requerimiento->firma_gerente_at)<div class="firma-fec">{{ $requerimiento->firma_gerente_at->format('d/m/Y H:i') }}</div>@endif
    </td>
  </tr>
</table>
