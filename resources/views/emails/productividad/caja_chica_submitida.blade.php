...
@section('alert_banner')
    <div class="email-alert {{ $esEdicion ? 'email-alert--warning' : 'email-alert--success' }}">
        {{ $esEdicion ? '✏️' : '💰' }}
        {{ $esEdicion ? 'Se actualizó un reporte de Caja Chica' : 'Nuevo reporte de Caja Chica recibido' }}
    </div>
@endsection
...
@section('body')
    ...
    <p class="email-intro">
        @if ($esEdicion)
            Se ha <strong>editado</strong> el reporte de Caja Chica de la sede <strong>{{ $reporte->sede }}</strong>.
            @if ($reporte->editado_tarde)
                <br><span style="color:#dc2626;font-weight:600;">⚠️ Edición realizada después del límite (sábado 2:00 PM). KPI afectado.</span>
            @endif
        @else
            Se recibió el <strong>reporte semanal de Caja Chica</strong> de la sede <strong>{{ $reporte->sede }}</strong>.
        @endif
    </p>
    ...
    <div class="data-card">
        ...
        <div class="data-card__body">
            ...
            <div class="data-row">
                ...
                <div class="data-value">
                    <span style="font-weight:700;font-size:16px;color:{{ $reporte->kpi_porcentaje >= 100 ? '#059669' : '#dc2626' }}">
                        {{ $reporte->kpiLabel() }}
                    </span>
                </div>
            </div>
    ...
    <div class="highlight-box">
        💡 <strong>KPI Caja Chica:</strong> Enviado el sábado = 100% · Cualquier otro día = 0%
    </div>
