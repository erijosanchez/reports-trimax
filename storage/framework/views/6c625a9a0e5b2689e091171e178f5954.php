...
<?php $__env->startSection('content'); ?>
                ...
                <div class="mb-4 card">
                    ...
                    <ul class="px-3 pt-2 nav nav-tabs" id="demandaTabs" role="tablist"
                        ...
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="tab-semanal"
                                    data-bs-toggle="tab" data-bs-target="#pane-semanal"
                        ...
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="tab-mensual"
                                    data-bs-toggle="tab" data-bs-target="#pane-mensual"
                                    type="button" role="tab"
                                    style="font-size:0.82rem; padding: 6px 14px;">
                    ...
                    <div class="tab-content card-body" style="border-top: 1px solid #dee2e6; padding-top: 16px;">
                        ...
                        <div class="tab-pane fade show active" id="pane-semanal" role="tabpanel">
                            ...
                            <div class="d-flex align-items-center gap-2 mb-3">
                                ...
                                <button onclick="exportarExcel('tablaSemanal', 'semanal')"
                                    ...
                                    <i class="me-1 mdi mdi-microsoft-excel"></i>Exportar Excel
                                </button>
                            </div>
                        ...
                        <div class="tab-pane fade" id="pane-mensual" role="tabpanel">
                            ...
                            <div class="d-flex justify-content-end mb-3">
                                <button onclick="exportarExcel('tablaMensual', 'mensual')"
    ...
    <script>
        /**
         * Exporta la tabla visible a un archivo .xlsx usando SheetJS.
         * Limpia el HTML de los badges (Cat.) antes de exportar.
         *
         * @param {string} tablaId  — id del <table>
         * @param {string} tipo     — 'semanal' | 'mensual'  (para el nombre del archivo)
         */
        function exportarExcel(tablaId, tipo) {
            const tabla = document.getElementById(tablaId);
            if (!tabla) return;

            // Clonar la tabla para no mutar el DOM original
            const clone = tabla.cloneNode(true);

            // Reemplazar el contenido de la columna Cat. (badges) por texto plano
            clone.querySelectorAll('td:first-child .badge, th:first-child').forEach(el => {
                const text = el.textContent.trim();
                const td   = el.closest('td') || el.closest('th');
                if (td) td.textContent = text;
            });

            // Limpiar cualquier otro badge residual en toda la tabla
            clone.querySelectorAll('.badge').forEach(badge => {
                const text = badge.textContent.trim();
                badge.replaceWith(document.createTextNode(text));
            });

            // Construir el workbook desde el HTML limpio
            const wb = XLSX.utils.book_new();
            const ws = XLSX.utils.table_to_sheet(clone);

            // Nombre de hoja y archivo según tipo
            const anio = document.getElementById('anioSelect').value;
            let nombreHoja, nombreArchivo;

            if (tipo === 'semanal') {
                const mesOpt = document.getElementById('mesSelect');
                const mes    = mesOpt.options[mesOpt.selectedIndex].text.trim();
                nombreHoja   = `Semanal ${mes} ${anio}`;
                nombreArchivo = `Demanda_Semanal_${mes}_${anio}.xlsx`;
            } else {
                nombreHoja    = `Mensual ${anio}`;
                nombreArchivo = `Demanda_Mensual_${anio}.xlsx`;
            }
        ...
        async function actualizarSemanal() {
            ...
            try {
                ...
                const mesOpt = document.getElementById('mesSelect');
                document.getElementById('labelMesSemanal').textContent = mesOpt.options[mesOpt.selectedIndex].text + ' ';
        ...
        function catBadge(cat) {
            const colors = { A: 'success', B: 'primary', C: 'warning', D: 'secondary' };
