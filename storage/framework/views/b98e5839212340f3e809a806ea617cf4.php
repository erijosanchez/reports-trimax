...
<?php $__env->startSection('content'); ?>
    ...
    <script>
        ...
        function renderTabla(clientes) {
            ...
            // ── Dentro de cada sede ordenar de mayor a menor total ────────
            Object.keys(grupos).forEach(sede => {
                grupos[sede].sort((a, b) => (b.total || 0) - (a.total || 0));
            });

            // ── Ordenar sedes de mayor a menor por su total acumulado ─────
            const sedesOrdenadas = Object.keys(grupos).sort((a, b) => {
                const sumA = grupos[a].reduce((s, c) => s + (c.total || 0), 0);
                const sumB = grupos[b].reduce((s, c) => s + (c.total || 0), 0);
                return sumB - sumA;
            });
