...
class CobranzaSedesController extends Controller
{
    ...
    public function index()
    {
        ...
        $mostrarExcepcionNota = !$user->isSede() || !ReporteCobranza::esSedeLimite11($user->sede);
    ...
    public function update(Request $request, ReporteCobranza $reporte)
    {
        ...
        // Reedición → vuelve a pendiente de revisión:
        // - Rechazado: siempre (para permitir la corrección).
        // - Conforme observado: SOLO si la reedición es dentro del plazo (a tiempo).
        //   Si se reedita después de vencido, se conserva la observación y la penalización.
    ...
    private function notificarRevision(ReporteCobranza $reporte, string $estado): void
    {
        $owner = $reporte->user;
        ...
        try {
            ...
            Log::error("Notificación de revisión (cobranza) no enviada: " . $e->getMessage());
        }
    }
    ...
    private function mapRevisionArchivos(ReporteCobranza $reporte): array
    {
        return collect($reporte->revision_archivos ?? [])->map(function ($a, $i) use ($reporte) {
            return [
                'name'         => $a['name'] ?? 'archivo',
                'es_imagen'    => str_starts_with($a['mime'] ?? '', 'image/'),
                'preview_url'  => route('productividad.cobranza-sedes.cobranza.revision-file', [$reporte->id, $i]),
                'download_url' => route('productividad.cobranza-sedes.cobranza.revision-file', [$reporte->id, $i]) . '?download=1',
            ];
        })->values()->all();
    }

    /** Sirve un adjunto de la revisión (inline por defecto, ?download=1 para bajar). */
    public function revisionFile(ReporteCobranza $reporte, int $index, Request $request)
    {
        if (!auth()->user()->puedeVerCobranzaSedes()) {
            abort(403);
        }
