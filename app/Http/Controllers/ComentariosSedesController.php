...
class ComentariosSedesController extends Controller
{
    ...
    public function index()
    {
        ...
        [$semanaNumero, $anio, , , $limiteCarbon] = ReporteComentarios::datosSemanActual();
        $fechaLimiteTs = $limiteCarbon->timestamp * 1000;

        $reporteSemanaActual = null;
        if ($user->isSede() && $user->sede) {
            $reporteSemanaActual = ReporteComentarios::obtenerOCrearSemanaActual($user->id, $user->sede);
        }
        ...
        $historialQuery = ReporteComentarios::with('user')
            ->orderByDesc('anio')
        ...
        return view('productividad.cobranza-sedes.comentarios', compact(
    ...
    public function store(Request $request)
    {
        ...
        $reporte = ReporteComentarios::obtenerOCrearSemanaActual($user->id, $sede);
    ...
    private function notificarRevision(ReporteComentarios $reporte, string $estado): void
    {
        $owner = $reporte->user;
        ...
        try {
            ...
            Log::error("Notificación de revisión (comentarios) no enviada: " . $e->getMessage());
        }
    }

    // ── show ──────────────────────────────────────────────────────

    public function show(ReporteComentarios $reporte)
    {
        ...
        $archivos = collect($reporte->archivos ?? [])->map(function ($a, $i) use ($reporte) {
            return array_merge($a, [
                ...
                'download_url' => route('productividad.cobranza-sedes.comentarios.download', [$reporte->id, $i]),
                'preview_url'  => route('productividad.cobranza-sedes.comentarios.preview',  [$reporte->id, $i]),
            ]);
        });
    ...
    private function mapRevisionArchivos(ReporteComentarios $reporte): array
    {
        return collect($reporte->revision_archivos ?? [])->map(function ($a, $i) use ($reporte) {
            return [
                'name'         => $a['name'] ?? 'archivo',
                'es_imagen'    => str_starts_with($a['mime'] ?? '', 'image/'),
                'preview_url'  => route('productividad.cobranza-sedes.comentarios.revision-file', [$reporte->id, $i]),
                'download_url' => route('productividad.cobranza-sedes.comentarios.revision-file', [$reporte->id, $i]) . '?download=1',
            ];
        })->values()->all();
    }

    /** Sirve un adjunto de la revisión (inline por defecto, ?download=1 para bajar). */
    public function revisionFile(ReporteComentarios $reporte, int $index, Request $request)
    {
        if (!auth()->user()->puedeVerCobranzaSedes()) abort(403);
    ...
    public function download(ReporteComentarios $reporte, int $index)
    {
        if (!auth()->user()->puedeVerCobranzaSedes()) abort(403);
    ...
    public function preview(ReporteComentarios $reporte, int $index)
    {
        if (!auth()->user()->puedeVerCobranzaSedes()) abort(403);
        $archivo = ($reporte->archivos ?? [])[$index] ?? null;
        if (!$archivo || !Storage::disk('local')->exists($archivo['path'])) abort(404);
    ...
    private function guardarArchivos(array $archivos, ReporteComentarios $reporte): array
    {
        $dir = "comentarios/{$reporte->anio}/semana-{$reporte->semana_numero}/{$reporte->sede}";
    ...
    private function getKpiChartData(int $semanas, ?string $sede = null): array
    {
        ...
        $query = ReporteComentarios::whereIn('semana_numero', array_column($semanasData, 'semana'))
    ...
    private function getKpiMensualData(int $meses, ?string $sede = null): array
    {
        ...
        $query = ReporteComentarios::whereNotNull('semana_inicio')
