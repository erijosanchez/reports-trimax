...
<div
    x-data="{
        ...
        async copyToClipboard() {
            try {
                await window.copyToClipboard('<?php echo $request->fullUrl(); ?>');
                this.copied = true;
    ...
    <?php echo $attributes->merge(['class' => "bg-white dark:bg-[#1a1a1a] border border-neutral-200 dark:border-white/10 rounded-lg flex items-center justify-between h-10 px-2 shadow-xs"]); ?>
...
<?php endif; ?>
            <?php echo $exception->httpStatusCode(); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'laravel-exceptions-renderer::components.http-method','data' => ['method' => ''.$request->method().'']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['method' => ''.$request->method().'']); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
...
<?php endif; ?>
        <div class="flex-1 text-sm font-light truncate text-neutral-950 dark:text-white">
            <span data-tippy-content="<?php echo $request->fullUrl(); ?>">
